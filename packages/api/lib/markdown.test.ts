...
/**
 * Eine Anleitung ist eine Spalte.
 *
 * `FORM-04` verlangt 65 Zeichen für den Fliesstext und *erlaubt* Tabellen und
 * Diagrammen, breiter zu laufen. Eine Weile war das als Auftrag gelesen: der
 * Fliesstext wurde Element für Element eingeengt, Codeblöcke und Tabellen
 * liefen über die volle Artikelbreite, und die Seite hatte drei Kanten. Die
 * Erlaubnis wird jetzt nicht genutzt - eingeengt wird der Container, und der
 * Renderer engt gar nichts mehr ein.
 */
describe("der Renderer engt nichts selbst ein", () => {
  test("kein Baustein bringt eine eigene Breite mit", () => {
    for (const quelle of [
      "> Hinweis",
      ":::klapp T\ndrin\n:::",
      "| a | b |\n|---|---|\n| 1 | 2 |",
      ...
      expect(renderMarkdown(quelle)).not.toContain("max-w-[65ch]");
    }
  });

  /**
   * Die Breite steht an einer Stelle, und die Vorschau im Editor benutzt
   * dieselbe - sonst bräche sie die Zeilen woanders als die Seite.
   */
  test("Seite und Vorschau teilen sich das Mass", async () => {
    const seite = await Bun.file(
      new URL("../frontend/pages/guides/page.tsx", import.meta.url),
    ).text();
    const editor = await Bun.file(
      new URL("../frontend/pages/management/guide-edit-page.tsx", import.meta.url),
    ).text();

    expect(seite).toContain('const MEASURE = "max-w-[65ch]"');
    expect(seite).toContain("<article class={MEASURE}>");
    // Und nichts engt mehr einzelne Elemente ein.
    expect(seite).not.toContain("[&>p]:max-w");
    expect(editor).toContain("max-w-[65ch]");
  });
});
