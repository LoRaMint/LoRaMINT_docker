...
The image is published at `ghcr.io/loramint/loramint_docker:latest`. The Docker
build uses `packages/api` as its build context (see `packages/api/Dockerfile`).

Deploy with Docker Compose (from the repository root, where `compose.prod.yml` lives):
...
The API package lives in `packages/api`. PostgreSQL is started from the
repository root (where `compose.dev.yml` lives); all other commands run inside
`packages/api`.
...
Starts a PostgreSQL 15 instance on `localhost:5432` with the credentials from `compose.dev.yml`.

**4. Install dependencies** (in `packages/api`)

```bash
bun install
```

**5. Run database migrations** (in `packages/api`)
...
**6. Start the dev server** (in `packages/api`)
...
preload.ts               Bun preload (registers the SSR plugin)
package.json             Package dependencies and scripts
bunfig.toml              Bun config (preload)
tsconfig.json            TypeScript config
Dockerfile               Production image (build context: packages/api)
...
public/
  global.css             Generated TailwindCSS bundle (build artifact)
  logo_loramint.svg      LoRaMINT logo
  logo_sfz.svg           SFZ logo
frontend/
  components/            Shared SolidJS components (layout)
  pages/
  ...
  styles/
    global.css           TailwindCSS + DaisyUI source
lib/
...
scripts/
  build-css.ts           Builds public/global.css from frontend/styles
...
To start with a fresh database (run from the repository root):
...
Then re-run the migrations from `packages/api`:
